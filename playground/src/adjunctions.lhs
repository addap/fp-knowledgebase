...
a.d. The only really interesting thing done with adjunctions here was zapWithAdjunction
because unit/counit/leftAdjunct/rightAdjunct was basically just index/tabulate since
every right-adjunct functor in Haskell is representable.
The following should be an implementation of zapWithAdjunction.
I guess the best intuition is that both functors hold one piece of the puzzle and
if we have both we can reduce it to something else

> zapWithAdjunction' :: (Adjunction f u) => (a -> b -> c) -> u a -> f b -> c
> zapWithAdjunction' zap ua = rightAdjunct (\b -> (flip zap b) <$> ua)
